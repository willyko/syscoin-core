...
function get(body) {
  ...
  if (body.error !== null) {
    throw new _rpcError2.default(_lodash2.default.get(body, 'error.code', -32603), _lodash2.default.get(body, 'error.message', 'An error occurred while processing the RPC call to syscoind'));
  }
...
class Parser {
  ...
  rpc(_ref3) {
    ...
    if (typeof body === 'string' && response.statusCode !== 200) {
      ...
      throw new _rpcError2.default(response.statusCode, JSON.stringify(JSON.parse(response.body).error));
    }
